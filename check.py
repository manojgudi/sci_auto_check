...
def isArray(func_name, content):
	...
	#print "all func" +str(all_func_invoked)
	...
	if count != len(all_func_invoked):
		#print "ARRAY "+func_name	
		pass
	else:
		print "FUNC: "+str(all_func_invoked) 
...
def extract_func(abs_file):
	...
	def rm_loops(list_of_func, content):
		...
		for func in list_of_func:
			if ((func != 'if') and (func != 'for') and (func != 'while') and (isArray(func,content))): # Replace last condition by array condition
				func = func.strip("(")
				proper_func_list.append(func)

		return proper_func_list

	list_of_func = rm_loops(list_of_func, file_content)
	return list_of_func
